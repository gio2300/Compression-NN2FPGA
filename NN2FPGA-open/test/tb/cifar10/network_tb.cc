...
/* Bytes per batch */
  ...
  if (getcwd(currentDirBuffer, FILENAME_MAX) != nullptr) {
    ...
    // Find the position of "NN2FPGA-open" in the path
    size_t topPos = currentDirStr.find("NN2FPGA-open");

    if (topPos != std::string::npos) {
      // Remove everything after NN2FPGA-open
      currentDirStr = currentDirStr.substr(0, topPos);
      currentDirStr += "NN2FPGA-open/work/";
